...
module.exports = bosco = async (bosco, mek) => {
	try {
       ...
       const catlo = (teks) => {
        ...
        const reply = (teks) => {
	      bosco.sendMessage(from, teks, text, { thumbnail: denis, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: false, "externalAdReply": { "title": `${' '}Sᴜʙsᴄʀɪʙᴇ Yᴛ : 𝐯𝐚𝐧𝐢𝐭𝐚𝐬${''}${''}`, "body": `Gʀᴏᴜᴘ Assɪsᴛᴇɴᴛ Bᴏᴛ`, "previewType": 'PHOTO', "thumbnailUrl": `${''}`, "thumbnail": denis, "sourceUrl": `${''}`}},})
        }
...
// AUTO
      ...
      const sendWebp = async(to, url) => {
       ...
       const sendMediaURL = async(to, url, text="", mids=[]) =>{
           ...
           bosco.sendMessage(to, media, type, {quoted: mek, "externalAdReply": { "title": `${' '}𝐚𝐝𝐝 𝐦𝐞 𝐢𝐧 𝐅𝐚𝐜𝐞𝐛𝐨𝐨𝐤: 𝐯𝐚𝐧𝐢𝐭𝐚𝐬 𝐱𝐟${''}${''}`, "body": `Gʀᴏᴜᴘ Assɪsᴛᴇɴᴛ Bᴏᴛ`, "previewType": 'PHOTO', "thumbnailUrl": `${''}`, "thumbnail": denis, "sourceUrl": `${''}`}, mimetype: mime, caption: text, thumbnail: Buffer.alloc(0), contextInfo: {"mentionedJid": mids}})
...
`
 ...
 menumessage = { contentText: `${teks}`, footerText: `تابعني على فايسبوك vanitas xf`, buttons: menubutton, headerType: 6, locationMessage: bosco2.message.locationMessage}
 ...
 //------------------< Extra >-------------------
      ...
      case 'tod':
        ...
        case 'anime':
              ...
              imageMsg = ( await bosco.prepareMessage(from, fs.readFileSync(`./${sender}.jpeg`), 'imageMessage', {thumbnail: Buffer.alloc(0)})).message.imageMessage
              buttonsMessage = {footerText:'© 𝒎𝒂𝒅𝒆 𝒃𝒚 𝒗𝒂𝒏𝒊𝒕𝒂𝒔 𝒙𝒇', imageMessage: imageMsg,
